...
This benchmark measures Kevala's typed `choice` API on pinned weight packs.
...
The [README comparison table](README.md#decision-accuracy-and-latency) uses the repeated
Apple M4 Max campaign, with [raw outputs](benchmarks/results/decisions-m4-max-2026-09-23/)
and a [generated full report](benchmarks/results/decisions-m4-max-2026-09-23/report.md).
...
and pass ranges without selecting only the fastest pass.

Eight models selected the same options as the historical NVIDIA snapshot. SemIf 2B changed
one perturbation decision in every M4 pass: `59898712e090848aa8b9::perm:rotate1`.
Its two leading M4 probabilities differ by only 0.000052. This near-tie changes the suite
score from 55.6% to 55.2%; it illustrates why scores should be measured on the stated runtime,
not copied from another backend. Option probabilities are not calibrated confidence.

## Historical NVIDIA results: September 22–24, 2026
...
The README comparison table is generated by `scripts/report-decisions.mjs` from the raw
per-request answers and wall timings. It reports an arithmetic mean, not a median or a
geometric mean of kernel profiles. The table also shows the minimum and maximum full-pass
means so workstation timing noise is visible rather than hidden by pooling. Missing, nonnumeric, zero, and negative wall timings
...
FP32-saturated tails directly outside [-10, 10], avoiding the large tanh arguments that
produced non-finite output on this Metal runtime, and cubic arithmetic for extreme finite
inputs. The [real GPU regression guard](dev/gemma-gelu-check.html)
fails before the fix and passes 348 gated values afterward, with maximum absolute error
4.77e-7 against a CPU reference. [Before/after evidence](docs/benchmarks/gemma-gelu-metal-2026-09-23.json)
is saved. The M4 comparison is rerun on the rebuilt WASM so different builds are not mixed.

The September 22 snapshot remains a limited regression comparison. Its build revision was
not captured, six fixture hashes were supplied after the run, and Firefox's queue-completion
steps obscure fine-grained speed differences. The reconstructed `campaign.json` labels these
gaps. It is not evidence for current-main performance. None of these fixtures has had independent
human label adjudication, and evaluation/training overlap has not been audited. A larger,
representative, human-reviewed holdout is needed before publishing general accuracy claims.
...
timer privacy settings are relaxed only in the temporary benchmark profile. The recorded
`clock.minDeltaMs` probes timer-callback scheduling, not direct clock resolution. Firefox queue completion can produce roughly 100 ms steps in
...
### Repeated all-model comparison

On the benchmark host, put every pinned Q8 pack in `tmp/` and build the current WASM. Use the
already running shared agent Chrome at port 9333; the runner uses hidden targets and never
launches or raises a browser. Keep other GPU jobs idle. Run on the host itself, not against a
remote CDP endpoint whose CPU and OS differ from the runner's machine.

```sh
pnpm build
pnpm serve --port=18093

KEVALA_BENCH_CDP=http://127.0.0.1:9333 node scripts/bench-decisions.mjs \
  http://127.0.0.1:18093/ benchmarks/results/decisions-linux-2026-09-23 --passes=3

node scripts/report-decisions.mjs \
  --results benchmarks/results/decisions-linux-2026-09-23/run-1 \
  --repeat-results benchmarks/results/decisions-linux-2026-09-23/run-2 \
  --repeat-results benchmarks/results/decisions-linux-2026-09-23/run-3 \
  --include-model bruv1-0.8b --include-model bruv1-4b \
  --no-references --output benchmarks/results/decisions-linux-2026-09-23/summary.json \
  --readme README.md
```

Choose a new output directory for each campaign; the runner refuses to overwrite an existing
one. When publishing a new README snapshot, commit its raw runs, campaign metadata, summary,
and `report.md` together. Update the decision-report command in `.github/workflows/ci.yml`
to check that campaign's summary and README with the same results, repeats, and reference
arguments; keep older campaigns as historical evidence. The generated table links to that
campaign's report rather than the older scores in this document.

The runner hashes the complete HTTP-served packs against the catalog before measuring, records
Chrome's actual GPU system information and the host CPU/OS, and fingerprints the served JS/WASM
against the local checkout before and after measurement. Each model uses five untimed warmup
decisions, 864 scored calls, batch size one, and no inference cache or profiled latency.
Three full passes run serially, then the reporter pools all 2,592 calls per model. It preserves
the 72 source groups for uncertainty estimates instead of counting repeated runs as new
independent tasks. Option-order flips are computed within each pass, not across repetitions.
Per-pass scores and mean latencies remain inspectable in `summary.json` and `report.md`;
copied artifact bytes cannot masquerade as a second measurement. BF16 comparisons are optional
and use the first pass when supplied.

For a partial local smoke, append `--models=laya,kev-0.8b --passes=1`. A partial run does not
produce a publishable all-model comparison. GPU profiling benchmarks are a different workload
and must not be substituted for this table's end-to-end wall latency.

If disk space is tight, append `--pack=hosted`. Complete pinned packs are streamed for hash
verification and model loading without saving local copies. Network and load time remain
outside the measured decision latency; expect a full pack download for each model and pass.

### Single-model and reference checks
...
Recompute the published scores, mean latencies, and README table from the saved outputs without
downloading weights:
...
Check the current M4 Max summary and README table:

```sh
node scripts/report-decisions.mjs \
  --results benchmarks/results/decisions-m4-max-2026-09-23/run-1 \
  --repeat-results benchmarks/results/decisions-m4-max-2026-09-23/run-2 \
  --repeat-results benchmarks/results/decisions-m4-max-2026-09-23/run-3 \
  --no-references --output tmp/m4-max-decision-summary.json \
  --check-summary benchmarks/results/decisions-m4-max-2026-09-23/summary.json \
