...
const unknown = positional.filter((arg) => arg !== "--wall");
if (unknown.length) {
...
for (const model of ["laya", "kev-0.8b"]) {
  ...
  const url = `${base}/bench.html#auto&backend=webgpu&pack=local&model=${model}&runs=5&profile=${profile}&unique=1&shapes=0,1,2`;
...
writeFileSync(resolve(out, "summary.json"), JSON.stringify({ metricMs, mode: modeLabel, result: resultKind, models }, null, 2) + "\n");
