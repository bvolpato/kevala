...
export class GpuKev {
  constructor(gpu, layout, cfg) {
    ...
    // query-tiled attention (8 tokens x 4 heads per workgroup) where the GPU has the tiled Laya kernel
    this.attnTile = !!gpu.attentionTile;
    this.wgsl = gpu.wgsl;
    this.name = gpu.name;
    this.cfg = cfg;
    ...
    for (const op of ops) {
      ...
      switch (op.k) {
        ...
        case "attn":
          ...
          if (this.attnTile) pass.dispatchWorkgroups(Math.ceil(T / 8) + S, 2);
          else pass.dispatchWorkgroups(T, 8);
          break;
        case "silumul": {
