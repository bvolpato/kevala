...
import { assertWasmPackSize } from "./kev-layout.js";
...
export async function loadFile(path, { flavor, cpuKernel = "auto", threads = "auto" } = {}) {
  ...
  const text = (p, n) => {
    const start = p >>> 0;
    return dec.decode(mem().subarray(start, start + n));
  };
  const check = (rc) => {
    if (rc) throw new Error(text(x.kevala_error_ptr(), x.kevala_error_len()));
  };
  const bytes = await readFile(path);
  const ptr = x.kevala_alloc(bytes.length) >>> 0;
  mem().set(bytes, ptr);
  ...
  const run = (requests) => {
    ...
    const p = x.kevala_alloc(b.length) >>> 0;
    mem().set(b, p);
    try {
